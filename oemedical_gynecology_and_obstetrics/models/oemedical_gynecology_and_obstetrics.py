...
class PatientPregnancy(orm.Model):
    ...
    _columns = {
        ...
        'lmp': fields.date(
            'LMP', help="Last Menstrual Period", required=True),
        'pdd': fields.function(
            _get_pregnancy_data, type='date',
            string='Pregnancy Due Date'),
        'prenatal_evaluations': fields.one2many(
            'oemedical.patient.prenatal.evaluation', 'name',
            'Prenatal Evaluations'),
        'perinatal': fields.one2many(
            'oemedical.perinatal', 'name', 'Perinatal Info'),
        'puerperium_monitor': fields.one2many(
            'oemedical.puerperium.monitor', 'name', 'Puerperium monitor'),
        ...
        'pregnancy_end_age': fields.function(
            _get_pregnancy_data, type='char', string='Weeks',
...
class PrenatalEvaluation(orm.Model):
    ...
    _columns = {
        ...
        'gestational_weeks': fields.function(
            _get_patient_evaluation_data, method=True,
            string="Gestational Weeks", type='float'),
        'gestational_days': fields.function(
            _get_patient_evaluation_data, method=True,
            string='Gestational days', type='integer'),
        ...
        'diabetes': fields.boolean(
            'Diabetes',
            help='Check this box if the mother has glucose intolerance or'
                 'diabetes'),
...
class OemedicalPerinatal(orm.Model):
    ...
    _columns = {
        ...
        'monitoring': fields.one2many(
            'oemedical.perinatal.monitor', 'name', string='Monitors'),
        'puerperium_monitor': fields.one2many(
            'oemedical.puerperium.monitor', 'name', 'Puerperium monitor'),
        'medications': fields.one2many(
            'oemedical.patient.medication', 'patient_id',
            string='Medications',),
...
class OeMedicalPatient(orm.Model):
    ...
    _columns = {
        ...
        # _get_pregnancy_info , string='Pregnant' , type='boolean' ),
        ...
        'breast_self_examination': fields.boolean(
            ...
            help="Check if patient does and knows how to self examine her"
                 " breasts"),
        'pap_test': fields.boolean(
            'PAP test',
            help="Check if patient does periodic cytologic pelvic smear"
                 " screening"),
        ...
        'gravida': fields.integer(
            'Gravida', help="Number of pregnancies"),
        'premature': fields.integer(
            'Premature', help="Premature Deliveries"),
        'abortions': fields.integer('Abortions'),
        'stillbirths': fields.integer('Stillbirths'),
        'full_term': fields.integer(
            'Full Term', help="Full term pregnancies"),
        'menstrual_history': fields.one2many(
            'oemedical.patient.menstrual_history', 'name',
            'Menstrual History'),
        'mammography_history': fields.one2many(
            'oemedical.patient.mammography_history', 'name',
            'Mammography History'),
        'pap_history': fields.one2many(
            'oemedical.patient.pap_history', 'name', 'PAP smear History'),
        'prenatal_evaluations': fields.one2many(
            'oemedical.patient.prenatal.evaluation', 'name',
            'Prenatal Evaluations'),
        'colposcopy_history': fields.one2many(
            'oemedical.patient.colposcopy_history', 'name',
            'Colposcopy History'),
        'pregnancy_history': fields.one2many(
            'oemedical.patient.pregnancy', 'name',
            'Pregnancies'),
    }
...
class PatientMenstrualHistory(orm.Model):
    ...
    _columns = {
        'name': fields.many2one(
            'oemedical.patient', 'Patient', readonly=True, required=True),
        'evaluation': fields.many2one(
            'oemedical.patient.evaluation', 'Evaluation'),
        'evaluation_date': fields.date(
            'Date', help="Evaluation Date",  required=True),
        'lmp': fields.date(
            'LMP', help="Last Menstrual Period", required=True),
...
class PatientMammographyHistory(orm.Model):
    ...
    _columns = {
        ...
        'last_mammography': fields.date(
            'Date', help="Last Mammography", required=True),
...
class PatientPAPHistory(orm.Model):
    ...
    _columns = {
        ...
        'last_pap': fields.date(
            'Date', help="Last Papanicolau", required=True),
        'result': fields.selection([
            ('negative', 'Negative'),
            ('c1', 'ASC-US'),
            ('c2', 'ASC-H'),
...
class PatientColposcopyHistory(orm.Model):
    ...
    _columns = {
        'name': fields.many2one(
            'oemedical.patient',
            'Patient', readonly=True, required=True),
        'evaluation': fields.many2one(
            'oemedical.patient.evaluation', 'Evaluation'),
        'evaluation_date': fields.date(
            'Date', help="Date"),
        'last_colposcopy': fields.date(
            'Date', help="Last colposcopy", required=True),
        'result': fields.selection([
            ('normal', 'normal'),
            ('abnormal', 'abnormal'),
            ], 'result',
