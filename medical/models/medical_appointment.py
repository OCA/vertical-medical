...
class MedicalAppointmentStage(orm.Model):
    ...
    _columns = {
        ...
        'is_default': fields.boolean('Default?', help="If checked, this stage will be selected when creating new appointments."),
    }
...
class MedicalAppointment(orm.Model):
    ...
    def _get_empty_appointments(self, cr, uid, physician_ids, institution_ids, date_start, date_end, context=None):
        ...
        domain = [('physician_id','in', physician_ids), ('patient_id','=',default_patient), ('date_end', '>', date_start),('appointment_date', '<', date_end)]
        if institution_ids:
            domain += [('institution_id','in',institution_ids)]
    ...
    def _remove_empty_clashes(self, cr, uid, excluded_ids, physician_ids, institution_ids, date_start, date_end, context=None):
        """ Remove empty appointments that clash with given one """

        # remove from list those ids in excluded_ids
        empty_appointments = set(self._get_empty_appointments(cr, uid, physician_ids, institution_ids, date_start, date_end, context=context)).difference(excluded_ids)
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        date_end = (datetime.strptime(date_start, "%Y-%m-%d %H:%M:%S") + timedelta(minutes=duration)).strftime("%Y-%m-%d %H:%M")
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if 'appointment_date' in vals or 'duration' in vals:
            ...
            current_appointments = self._get_appointments(cr, uid, [physician_id], [], date_start, date_end, context=context).remove(ids[0])
            if current_appointments:
                raise orm.except_orm(_('Error!'), _('Appointment clashes with other'))
