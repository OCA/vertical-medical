...
class OeMedicalPathology(orm.Model):
    ...
    _columns = {
        'category': fields.many2one(
            'oemedical.pathology.category', string='Main Category',
        ...
        'info': fields.text(
            string='Extra Info'),
        'code': fields.char(
            size=256, string='Code', required=True,
            help='Specific Code for the Disease (eg, ICD-10)'),
        'name': fields.char(
            size=256, string='Name', required=True, translate=True,
            help='Disease name'),
        'groups': fields.one2many(
            'oemedical.disease_group.members', 'disease_group_id',
            string='Groups',
            help='Specify the groups this pathology belongs. Some'
                 ' automated processes act upon the code of the group'),
        'protein': fields.char(
            size=256, string='Protein involved',
            help='Name of the protein(s) affected'),
        'gene': fields.char(
            size=256, string='Gene'),
        'chromosome': fields.char(
            size=256, string='Affected Chromosome', help='chromosome number'),
    }
#    _sql_constraints = [
