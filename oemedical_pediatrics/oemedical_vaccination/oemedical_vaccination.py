...
class OeMedicalVaccination(orm.Model):
    ...
    _columns = {
        ...
        'vaccine_lot': fields.char(
            size=256, string='Lot Number',
            help='Please check on the vaccine (product) production lot '
                 'number and tracking number when available !'),
        'patient_id': fields.many2one('oemedical.patient', string='Patient',
                                      readonly=True),
        'vaccine': fields.many2one(
            'product.product', string='Vaccine',
            required=True,
            help='Vaccine Name. Make sure that the vaccine (product) has all'
                 ' the proper information at product level. Information such'
                 ' as provider, supplier code, tracking number, etc.. This'
                 ' information must always be present. If available, please'
                 ' copy / scan the vaccine leaflet and attach it to this '
                 ' record'),
