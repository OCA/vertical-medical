...
class OeMedicalPatient(orm.Model):
    ...
    _columns = {
        ...
        'sex': fields.selection(
            [('m', 'Male'), ('f', 'Female'), ], string='Sex', required=True),
        'general_info': fields.text(
            string='General Information',
            help='General information about the patient'),
        'dob': fields.date(
            string='DoB'),
        'medical_center_ids': fields.one2many(
            'oemedical.patient.med.center.rel', 'patient_id',
            'Related Medical Centers', readonly=True),
        'age': fields.function(
            _get_age, type='char', string='Age',
            help="It shows the age of the patient in years(y), months(m) and"
                 " days(d).\nIf the patient has died, the age shown is the age"
                 " at time of death, the age corresponding to the date on the"
                 " death certificate. It will show also \"deceased\" on the"
                 " field", multi=False),
        ...
        'active': fields.boolean(
            'Active',
            help="If unchecked, it will allow you to hide the patient without"
                 " removing it."),
    }
